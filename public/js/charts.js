...
jQuery(document).ready(function(){		
		...
		var ques=jQuery(".widgetcontent").attr("id");
		var ans=jQuery("#answer").val();
		var unans=jQuery("#unanswer").val();
		//alert(unans);
		...
		var series = 3;
		var question0=50;
		var question1=30; //answer
		var question2=10; //unanswer
		
		//for( var i = 0; i<series; i++) {
			//var question=2*10;
			data[0] = { label : " Total ("+(ques)+")", data:Math.floor(ques*10)}
			data[1] = { label : " Compleated ("+(ans)+")", data:Math.floor(ans*10)}
			data[2] = { label : " Pending ("+(unans)+")", data:Math.floor(unans*10)}
		//}
